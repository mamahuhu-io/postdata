...
const shortcodeBaseURL = "https://postdata.cn"

const copyEmbed = () => {
  return `<iframe src="${shortcodeBaseURL}/e/${props.request?.id}" title="Postdata Embed" style="width: 100%; height: 480px; border-radius: 4px; border: 1px solid rgba(0, 0, 0, 0.1);"></iframe>`
}
...
const copyButton = (
  ...
  if (type === "markdown") {
    return `[![Run in Postdata](${shortcodeBaseURL}/${badge})](${shortcodeBaseURL}/r/${props.request?.id})`
  }
  return `<a href="${shortcodeBaseURL}/r/${props.request?.id}"><img src="${shortcodeBaseURL}/${badge}" alt="Run in Postdata" /></a>`
}
...
const copyLink = (variationID: string) => {
  if (variationID === "link1") {
    ...
    return `<a href="${shortcodeBaseURL}/r/${props.request?.id}">Run in Postdata</a>`
  }
  return `[Run in Postdata](${shortcodeBaseURL}/r/${props.request?.id})`
}
