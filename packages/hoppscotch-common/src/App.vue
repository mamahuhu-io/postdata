<template>
  <div>
    ...
    <RouterView v-else />
    <Toaster rich-colors />
    <AppAbout :show="showAboutStatus" @hide-modal="showAboutStatus = false" />
    <AppUpgrade
      :show="showUpgradeStatus"
      @hide-modal="showUpgradeStatus = false"
    />
  </div>
</template>
...
import { Toaster } from "@mamahuhu/ui"
...
const t = useI18n()
const showAboutStatus = ref(false)
const showUpgradeStatus = ref(false)
...
const showAbout = () => {
  showAboutStatus.value = true
}

const showUpgrade = () => {
  showUpgradeStatus.value = true
}

ipcListener(router, toast)
listenGlobalAbout(showAbout)
listenGlobalUpgrade(showUpgrade)
</script>
